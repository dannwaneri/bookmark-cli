...
import { rewriteTweet, suggestReplies, summarizePattern, continueThread, learnStanceFromCorpus } from "./claude.js";
...
export async function handleWinners(chatId, env) {
  ...
  const list = winners.slice(0, 10).map((w, i) => `${i + 1}. "${w}"`).join("\n\n");
  await sendMessage(TELEGRAM_BOT_TOKEN, chatId,
    `${b(`Your top ${Math.min(winners.length, 10)} winners:`)}\n\n${list}`
  );
}
